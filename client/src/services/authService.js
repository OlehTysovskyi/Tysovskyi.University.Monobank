...
// TODO: Move auth functionality into separate file
...
export const useAuthentication = () => {
    ...
    const register = async (formData) => {
        try {
            const response = await fetch(
                'https://tysovskyi-university-monobank-server.vercel.app/api/register',
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const handleGoogleAuth = async () => {
        try {
            window.open(
                'https://tysovskyi-university-monobank-server.vercel.app/auth/google',
                '_self'
            );
            console.log('Login successful');
        } catch (error) {
    ...
    const fetchUserProfile = async () => {
        try {
            const response = await fetch(
                'https://tysovskyi-university-monobank-server.vercel.app/profile',
                {
                    credentials: 'include',
                }
            );
